...
// capture_pc_lifecycle.py checks the executable hash before invoking this.
...
int main(int argc,char **argv) {
 ...
 for(int entry=0;entry<2;entry++)for(int profile=0;profile<6;profile++)for(int angle=0;angle<360;angle+=59) {
  ...
  wrf(s,0x2934,1234);wr32(s,0x575c,1);wr32(s,0x19c,320);wrf(s,0x194,80);
  wr32(s,0x298c,angle);wr32(s,0x2990,angle);s[0x45]=1;
  if (entry) { wr32(s,0x54c4,-2);wrf(s,0x194,0);wr32(s,0x5504,profile?6-profile:0);
   for(int i=0;i<3;i++)wr32(s,0x54d0+i*4,1);
  }
  input(s);
  printf("S %d %d %d %.0f %u %u %.0f %u\n",entry,profile,angle,rdf(s,0x194),rd32(s,0x298c),rd32(s,0x2990),rdf(s,0x2934),rd32(s,0x19c));
  s[0x45]=0;logic(s);
  printf("F %d %d %d %.0f %u %.0f %u\n",entry,profile,angle,rdf(s,0x194),rd32(s,0x2990),rdf(s,0x2934),rd32(s,0x19c));
 }
 for(int profile=0;profile<6;profile++)for(int retry=0;retry<3;retry++) {
  ...
  wrf(s,0x2934,123);wrf(s,0x194,1);wr32(s,0x2990,137);wr32(s,0x298c,137);
  for(int tick=1;tick<=85;tick++) {
   s[0x45]=retry==1 || (retry==2 && tick==72);
   input(s);logic(s);
   printf("D %d %d %d %.0f %u %.0f %u\n",profile,retry,tick,rdf(s,0x194),rd32(s,0x19c),rdf(s,0x2934),rd32(s,0x2990));
  }
 }
}
