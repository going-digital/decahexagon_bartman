...
int main(int argc,char **argv) {
 assert(argc==2);load(argv[1]);unsigned char *s=calloc(1,0x6000);
 wrd(s,0x2948,1.0);
 for(int mode=0;mode<=10;mode++)for(int angle=-60;angle<=60;angle++) {
  wr32(s,0x2988,mode);wrf(s,0x54b0,angle);
  ((void(*)(void*))(0x10000aa70+slide))(s);
  printf("R %d %d %.0f\n",mode,angle,rdf(s,0x54b0));
 }
 for(int angle=-60;angle<=60;angle++) {
  wrf(s,0x54b0,angle);((void(*)(void*))(0x10000ab30+slide))(s);
  printf("N 0 %d %.0f\n",angle,rdf(s,0x54b0));
  for(int target=0;target<=30;target+=10) {
   wrf(s,0x547c,angle);((void(*)(void*,int))(0x10000aa10+slide))(s,target);
   printf("S %d %d %.0f\n",target,angle,rdf(s,0x547c));
  }
 }free(s);return 0;
}
