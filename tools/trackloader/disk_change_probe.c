/* Execute original DiskIO with a modeled change latch cleared by stepping.
 * No MFM data is supplied: reaching read DMA proves only a missed change,
 * not a successful replacement-media write. Timers expire immediately. */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
...
static unsigned scenario,prb=255,dma_starts,timer_starts,changed,steps,dma_complete,dma_active;
unsigned int m68k_read_memory_8(unsigned int a) {
 if(a<sizeof(mem))return mem[a];
 if(a==0xbfe001)return changed ? 0xeb : 0xef;
 if(a==0xbfd100)return prb;
 if(a==0xbfde00)return 0; /* one-shot timer expired */
 if(a==0xdff01f && dma_complete)return 2;
...
void m68k_write_memory_8(unsigned int a,unsigned int v){
 ...
 if(a==0xbfd100){
  if(!(prb&1) && (v&1) && !(v&8)){steps++;if(scenario==1)changed=0;}
  prb=v;
 }
 if(a==0xbfd500)timer_starts++;
 if(a==0xdff024){dma_active=(v&0x80)!=0;if(dma_active)dma_starts++;}
...
int main(int argc,char **argv){
 ...
 for(scenario=0;scenario<3;scenario++) {
  ...
  prb=255;dma_starts=timer_starts=steps=0;changed=scenario!=0;
  /* The wrapper has already checked the old medium. Model replacement just
   * after that check, before DiskIO seeks from cylinder zero to the save. */
  m68k_write_memory_16(0x1000+0x774+(guarded?16:0),0);
  if(guarded)mem[0x1000+n-2]=1;
  ...
  int expected=(scenario==2 || (guarded && scenario==1))?29:-1;
  ...
  if((prb&0x78)!=0x78)return 1; /* drive deselected by DiskIO cleanup */
  if((scenario==2 || (guarded && scenario==1)) ? dma_starts!=0 : (!steps || dma_starts==0))return 1;
  printf("{\"scenario\":%u,\"error\":%d,\"cycles\":%llu,\"steps\":%u,\"dma_starts\":%u,\"destination_unchanged\":true,\"drive_deselected\":true}\n",scenario,result,(unsigned long long)cycles,steps,dma_starts);
 }
 return 0;
}
