#!/usr/bin/env python3
"""Verify the NTSC native save and read-only cold-reboot artifacts."""
import hashlib
import json
import struct
import zlib
...
saved = (p/'ntsc_save.adf').read_bytes()
reboot = (p/'ntsc_reboot.adf').read_bytes()
...
for name, expected in {
    'ntsc_save': ['NATIVE-SAVE-DEFAULT','NATIVE-GAME-ENTRY','DF0:courtesyNATIVE-TUNE-READY'],
    'ntsc_reboot': ['NATIVE-SAVE-RESTORED','NATIVE-GAME-ENTRY'],
}.items():
    assert (p/(name+'.log')).read_text().splitlines() == expected
    assert (p/(name+'.png')).exists()
report = dict(status='NTSC native image boots, saves and restores on cold reboot',
              source_sha256=build['adf_sha256'], saved_sha256=hashlib.sha256(saved).hexdigest(),
              emulator='Copperline NTSC A500OCS, 512K Chip + 512K slow',
              changed_sectors=changed, generation=1, best_ticks=best[0],
              stage_bytes=build['stage_bytes'], reboot_disk_unchanged=True,
              screenshots=['ntsc_save.png','ntsc_reboot.png'],
...
(root/'docs/TRACKLOADER_NTSC_SAVE_RESULTS.json').write_text(json.dumps(report,indent=2)+'\n')
