...
parser=argparse.ArgumentParser();parser.add_argument('--whdload',action='store_true');args=parser.parse_args()
out=root/('scratchpad/whdload/game' if args.whdload else 'scratchpad/trackloader/native_game');out.mkdir(parents=True,exist_ok=True)
...
if args.whdload:flags+=['-DWHDLOAD=1']
# Storage transactions run only while loading/saving, not in the frame or IRQ path.
size_sources={'trackloader/save.c','trackloader/save_disk.c','trackloader/native_save.c','trackloader/tune_cache.c'}
objects=[]
...
(out/'link_results.json' if args.whdload else root/'docs/TRACKLOADER_NATIVE_GAME_RESULTS.json').write_text(json.dumps(report,indent=2)+'\n')
print(sections)
