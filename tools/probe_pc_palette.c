...
int main(int argc,char **argv) {
    ...
    int ids[]={0,1,3,5,6,7,9,10,11,12,13,14,15,30};
    for(unsigned i=0;i<sizeof(ids)/sizeof(*ids);i++) {
        memset(g,0,0x3000);setpal(g,ids[i],0);
        printf("SCHEME %d",ids[i]);
        for(int end=0;end<2;end++)for(int slot=0;slot<=2;slot+=2)
            for(int c=0;c<3;c++)printf(" %d",(int)rd32(g,0x25f0+end*0x78+slot*4+c*40));
        puts("");
    }
    ...
    install_hooks();
    for(int i=0;i<3;i++)memcpy((void*)(pal[i]+slide),saved[i],12);
    void (*logic)(void*)=(void*)(0x100056f50+slide);
    for(int stage=0;stage<3;stage++) {
        init_state(s);wr32(s,0x54c4,stage);setpal(g,stage==0?0:stage==1?5:10,1);
        for(int tick=1;tick<=7300;tick++) {
            wrf(s,0x2934,tick-1);wrf(s,0x296c,100000);logic(s);
            printf("T %d %d %d %d %d %d",stage,tick,(int)rd32(s,0x129f4),(int)rd32(s,0x129f0),(int)rdf(s,0x1b8),(int)rd32(s,0x1a0));
            for(int slot=0;slot<=2;slot+=2)for(int c=0;c<3;c++)printf(" %d",(int)rd32(g,0x26e0+slot*4+c*40));
            puts("");
        }
    }
    return 0;
