...
int main(int argc,char**argv){
 ...
 unsigned times[]={0,24500,163499,163500,167999,168000,168001};
 for(unsigned t=0;t<7;t++)for(unsigned marker=0;marker<2;marker++)for(unsigned delay=0;delay<4;delay++){
 init_state(s);wr32(s,0x54c4,4);wr32(s,0x57f8,15);wrf(s,0x296c,delay);
 wr32(s,0x551c,marker);wr32(s,0x41180,times[t]);wr32(s,0x5808,0);wrf(s,0x2934,7201);
 seed=0;draws=nchosen=0;((void(*)(void*))(0x100056f50+slide))(s);
 printf("%u %u %u %.0f %u %d\n",times[t],marker,delay,rdf(s,0x296c),rd32(s,0x54c4),nchosen?chosen[0]:-1);
 }free(s);return 0;
}
