...
int main(int argc,char**argv){
 ...
 void (*setpal)(void*,int,int)=(void*)(0x10004dab0+slide);
 unsigned char *s=calloc(1,0x50000),*g=s+0x10298;
 setpal(g,200,0);printf("SCHEME 200");
 for(int end=0;end<2;end++)for(int slot=0;slot<=2;slot+=2)for(int c=0;c<3;c++)printf(" %d",(int)rd32(g,0x25f0+end*0x78+slot*4+c*40));puts("");
 unsigned char saved[3][12];uintptr_t pal[]={0x10004c8b0,0x10004dab0,0x10004f1a0};
 for(int i=0;i<3;i++)memcpy(saved[i],(void*)(pal[i]+slide),12);
 install_hooks();for(int i=0;i<3;i++)memcpy((void*)(pal[i]+slide),saved[i],12);
 init_state(s);wr32(s,0x54c4,4);wr32(s,0x57f8,15);setpal(g,30,1);
 int targets[]={9,10,7,6,5,1,3,0,200,30};
 for(unsigned tick=1;tick<=1000;tick++){
 wrf(s,0x296c,100000);((void(*)(void*))(0x100056f50+slide))(s);
 if(tick%100==1){wr32(s,0x129f8,targets[tick/100]);wr32(s,0x129f0,1);}
 printf("T %u %u %u %.0f %u",tick,rd32(s,0x129f4),rd32(s,0x129f0),rdf(s,0x1b8),rd32(s,0x1a0));
 for(int slot=0;slot<=2;slot+=2)for(int c=0;c<3;c++)printf(" %u",rd32(g,0x26e0+slot*4+c*40));puts("");
 }free(s);return 0;
}
