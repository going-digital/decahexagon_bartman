#!/usr/bin/env python3
"""Execute ending reference on macOS; emit fixture text on stdout.
Requires the owned, matching PC executable and x86-64 execution support.
Usage: python3 tools/capture_pc_ending.py [path/to/SuperHexagon]
"""
...
exe=root/'out/pc_ending_probe'
subprocess.run(['clang','-arch','x86_64','-O0',str(root/'tools/probe_pc_ending.c'),'-o',str(exe)],check=True)
